...
indiaRasterReprojected <- raster(x = indiaExtent, nrows = nrow(indiaTemperaturesLastWeek[[1]]), ncols = ncol(indiaTemperaturesLastWeek[[1]]), crs = crs(indiaPolygons)) # A bug appears when we use crs(cityPoints) (like before) instead of crs(indiaPolygons) (fixed). The printout for both crs appears to be the same, but we get 
# > identical(crs(indiaPolygons), crs(cityPoints))
# [1] FALSE
# This bug results almost surely from the update in gdal.
...
lapply(c("2012-05-27", "2012-05-28", "2012-05-29"), function(dateName) {
  ...
  # originRasterAsPoints <- raster::rasterToPoints(indiaTemperaturesLastWeek[[dateName]], spatial=T)
  # transformedPoints <- sp::spTransform(originRasterAsPoints, CRSobj = crs(indiaRasterReprojected))
  # rasterReprojected <- raster::rasterize(transformedPoints, y = indiaRasterReprojected, field = as.vector(originRasterAsPoints@data))
...
# pointsToTransfer <- raster::rasterToPoints(temperaturesMay21, spatial = TRUE)
# temperaturesMay21reproj <- raster::rasterize(x = pointsToTransfer, y = temperaturesMay21, field = raster::values(temperaturesMay21)[!is.na(raster::values(temperaturesMay21))])
